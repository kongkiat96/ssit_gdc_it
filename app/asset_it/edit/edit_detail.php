...
<div class="modal-body">
   <div class="form-group row">
      ...
      <div class="col-md-4">
         ...
         <input type="text" name="edit_card_location" id="card_location" class="form-control input-sm" placeholder="Asset location" required value="<?php echo $getuse_detail->card_location; ?>">
         <div class="invalid-feedback">
            ระบุ สถานที่ติดตั้ง.
         </div>
      </div>
   ...
   <div class="form-group row">
      <div class="col-md-6 col-sm-12">
         ...
         <select name="edit_card_company" id="company" class="form-control select2bs4 input-sm" required="">
            <option value="">--- เลือกข้อมูล ---</option>
            <?php
            $getcompany = $getdata->my_sql_select($connect, NULL, "company", "id AND cp_status = '1'");
      ...
      <div class="col-md-6 col-sm-12">
         ...
         <select name="edit_card_branch" id="branch" class="form-control select2bs4 input-sm" required="">
            <option value="">--- เลือกข้อมูล ---</option>
            <?php
            $getbranch = $getdata->my_sql_select($connect, NULL, "branch", "id AND status = '1'");
            while ($showprefix = mysqli_fetch_object($getbranch)) {
               if ($showprefix->id == $getuse_detail->card_branch) {
                  echo '<option value="' . $showprefix->id . '" selected>' . $showprefix->branch_name . '</option>';
               } else {
                  echo '<option value="' . $showprefix->id . '">' . $showprefix->branch_name . '</option>';
               }
            }
            ?>
         </select>
   ...
   <div class="form-group row">
      <div class="col-md-6 col-sm-12">
         <label for="card_equipment">อุปกรณ์</label>
         <select name="edit_card_equipment" id="card_equipment" class="form-control select2bs4 input-sm" required="">
            <option value="">--- เลือกข้อมูล ---</option>
            <?php
            $getdevice = $getdata->my_sql_select($connect, NULL, "device_type", "device_status = '1' ORDER BY id");
            while ($showprefix = mysqli_fetch_object($getdevice)) {
               if ($showprefix->id == $getuse_detail->card_equipment) {
                  echo '<option value="' . $showprefix->id . '" selected>' . $showprefix->device_type . '</option>';
               } else {
   ...
   <div class="form-group row">
      <div class="col-md-6">
         <label for="edit_card_customer_phone">เบอร์โทรศัพท์</label>
      ...
      <div class="col-md-6">
         <label for="edit_card_customer_email">อีเมล</label>
         <input type="text" name="edit_card_customer_email" id="edit_card_customer_email" class="form-control input-sm" placeholder="อีเมล" value="<?php echo @$getuse_detail->card_customer_email; ?>">
      </div>
   </div>
   <div class="form-group row">
      <div class="col-12">
   ...
   <div class="form-group row">
      <div class="col-6">
         <input type="text" name="license_name" id="os" class="form-control input-sm" placeholder="Windows / Other" value="<?php echo @$getuse_detail->license_name; ?>">
      </div>
      <div class="col-6">
